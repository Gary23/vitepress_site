...
<script lang="ts" setup name="Person">
  ...
  let person = reactive({     // 调用reactive返回一个Proxy对象
    name: '张三',
    age: 18,
    tel: '123456789'
  })

  let person2 = ref({     // 调用ref返回一个refImpl对象，person2.value是一个Proxy对象，通过value修改就是通过reactive实现的
    name: 'tom',
    age: 18,
    tel: '33990022'
  })
  ...
  function changeName() {
    person.name = '李四'      // Proxy对象修改时候直接修改属性值就可以
  }
  function changeAge() {
    person.age += 1
  }
  ...
  function changeName2() {
    person2.value.name = 'jack'    // refImpl对象修改时候需要修改value的值
  }
  function changeAge2() {
    person2.value.age += 1
  }
